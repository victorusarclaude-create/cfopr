...
function sheetSubj(sid){
  ...
  const gs=ARENA.filter(g=>g.s===sid).map(g=>({id:g.id,title:g.nome+' · nível '+arUnl(g.id)+'/5'})).concat(GAMES.filter(g=>g.s===sid));
...
function sheetTopic(tid,focus){
  ...
  UI.topicOpen=tid;
  const st=T(tid), tm=topicMastery(tid), note=S.notes[tid], cards=(S.flashcards[tid]||[]).length, games=ARENA.filter(g=>g.t===tid).map(g=>({id:g.id,title:g.nome+' · nível '+arUnl(g.id)})).concat(GAMES.filter(g=>g.t===tid)), bank=(S.qbank[tid]||[]).length;
