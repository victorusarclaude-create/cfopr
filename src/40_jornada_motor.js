...
/* voz: escolhe a mais natural do aparelho (neurais/online/"premium" primeiro) e lê frase por frase,
   com pausas curtas, o que soa menos robótico e evita o corte de falas longas no Chrome */
function jnVoice(){
  const vs=(window.speechSynthesis.getVoices()||[]).filter(v=>/^pt/i.test(v.lang)); if(!vs.length) return null;
  const sc=v=>(/^pt[-_]BR/i.test(v.lang)?50:0)+(/natural|neural|online|premium|enhanced|aprimorad|wavenet/i.test(v.name)?40:0)+(/google/i.test(v.name)?25:0)+(/francisca|antonio|thalita|luciana|felipe|camila|vitoria|daniel/i.test(v.name)?15:0)+(v.localService?0:5);
  return vs.slice().sort((a,b)=>sc(b)-sc(a))[0];
}
function jnSpeak(t){ try{ const sp=window.speechSynthesis; if(!sp||!t) return; sp.cancel();
  const txt=stripQ(t).replace(/[“”"]/g,'').replace(/\s*→\s*/g,', depois ').replace(/\bséc\.\s*/gi,'século ').replace(/\bnº\s*/gi,'número ').replace(/(\d)\.(\d{3})/g,'$1$2');
  const parts=txt.split(/\n+|(?<=[.!?:;])\s+/).map(x=>x.trim()).filter(Boolean);
  const v=jnVoice();
  parts.forEach((x,i)=>{ const u=new SpeechSynthesisUtterance(x); u.lang=v?v.lang:'pt-BR'; if(v) u.voice=v; u.rate=v&&!v.localService?1:0.95; u.pitch=1.03; u.volume=1; sp.speak(u); });
}catch(e){} }
try{ if(window.speechSynthesis) speechSynthesis.addEventListener('voiceschanged',()=>{}); }catch(e){}
function jnNarrate(){
